...
##########
# temp
from job_dash_prep_data import *
###
...
####################################
job_names = [(jobs[job].title, job) for job in range(len(jobs)-1)]
...
tabs = html.Div([
    dcc.Tabs(id="tabs", value='tab-1', children=[
        dcc.Tab(label='Job description', value='tab-1'),
        dcc.Tab(label='Other info', value='tab-2'),
    ]),
    html.Div(id='tabs-content')
])
...
@app.callback([
    #dash.dependencies.Output('tabs', 'children'), ##TODO: tabs need more work to integrate
...
dash.dependencies.State("dd_value", 'data')])
#[dash.dependencies.Input('toggle_dropdown_sort', 'on')])
#####################################
@cache.memoize(timeout=TIMEOUT) #make a separate data query function
def update_output(id, sort_toggle, logo_dict, total_jobs_badge, dd_value):
    spinner_text = ""
    ...
    if job_id is None:
        ...
        return [""]*4 + [logo_dict, init_plot, total_jobs_badge, spinner_text, None]
        raise PreventUpdate

    logo_dict |= dict(display="inline-block")
    total_jobs_badge |= dict(display="none")

    recency_status = update_recency(jobs[job_id].url)
    closed_msg = ""
    ...
    if sort_toggle is False:
        return [
            #"***\n"+ \
            unsorted_jobs[job_id].description,
            unsorted_jobs[job_id].title,
            f"##### **Score:** {round(unsorted_jobs[job_id].score,1)}",
            unsorted_jobs[job_id].url,
            logo_dict,
            "",
            total_jobs_badge,
            spinner_text,
